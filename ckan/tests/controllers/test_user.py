...
class TestUser(object):
    ...
    def test_register_a_user(self, app: helpers.CKANTestApp):
        ...
        assert response['Cache-Control'] == 'no-cache, no-store, max-age=0'
        assert response.cache_control.to_header() == 'no-cache, no-store, max-age=0'
        ...
        assert response.cache_control.must_revalidate is False, response
